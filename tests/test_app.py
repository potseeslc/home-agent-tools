...
def session(who):
    payload = (
        base64.urlsafe_b64encode(
            json.dumps({"token_use": "session", "email": who, "sub": who}).encode()
        )
        .decode()
        .rstrip("=")
    )
...
class Broker:
    ...
    def __call__(self, request):
        ...
        tools = [
            {
                ...
                "name": "ha-api-status",
                **ownership,
                "authToken": "NEVER-SEND-THIS",
            },
            {"id": "git", "name": "gitea-personal-evaluation-get-me", **ownership},
            {"id": "repos", "name": REPOS, **ownership},
        ]
        if path == "/tools":
        ...
        if path == "/gateways":
            return httpx.Response(
                ...
                json=[
                    {
                        "id": "gitea",
                        "name": "gitea-personal-evaluation",
                        **ownership,
                        "oauthConfig": {"client_secret": "NEVER-SEND-THIS"},
                    }
                ],
            )
        if path == "/servers":
            return httpx.Response(
                ...
                json=[
                    {
                        "id": who + "-server",
                        "name": "Home Agent Tools Evaluation",
                        **ownership,
                    }
                ],
            )
        if path == "/tokens" and request.method == "POST":
            ...
            self.counter += 1
            new = "test-token-" + str(self.counter)
            self.tokens[new] = {"owner": who, "active": True, "id": str(self.counter)}
            return httpx.Response(
                201, json={"token": {"id": str(self.counter)}, "access_token": new}
            )
        if path.startswith("/tokens/") and request.method == "DELETE":
            for row in self.tokens.values():
                if row["id"] == path.split("/")[-1] and row["owner"] == who:
                    row["active"] = False
            return httpx.Response(204)
        if path == "/auth/logout":
            return httpx.Response(200, json={})
        if path == "/oauth/authorize/gitea":
            return httpx.Response(
                302,
                headers={"location": "https://git.example.com/login/oauth/authorize"},
            )
        if path == "/rpc":
            body = json.loads(request.content)
            ...
            result = (
                {"message": "API running."}
                if name == "ha-api-status"
                else {
                    "id": 7,
                    "login": self.account if who == "alice" else "bob-upstream",
                }
            )
            return httpx.Response(
                200,
...
async def check(client, service):
    return await client.post(
        "/api/connections/" + service + "/test", headers=await headers(client)
    )
...
async def enroll(client, services=None):
    ...
    for service in services:
        assert (await check(client, service)).json()["ok"]
    r = await client.post(
        "/api/agents",
        headers=await headers(client),
        json={"name": "Test agent", "services": services, "days": 1},
    )
...
async def rpc(client, token, method="tools/call", params=None):
    return await client.post(
        ...
        json={
            ...
            "id": 1,
            "method": method,
            "params": params or {"name": "ha-api-status", "arguments": {}},
        },
    )
...
async def test_malformed_calls_and_repository_bounds(setup):
    ...
    for args in ({"limit": 1}, {"per_page": 51}, {"page": 0}, {"per_page": 1.5}):
        assert (
            await rpc(c, a["token"], params={"name": REPOS, "arguments": args})
        ).json()["error"]["code"] == -32602
    assert (
        "result"
        in (
            await rpc(
                c, a["token"], params={"name": REPOS, "arguments": {"per_page": 1}}
            )
        ).json()
    )
    assert (await rpc(c, a["token"], params={"name": [], "arguments": {}})).json()[
        "error"
    ]["code"] == -32602
...
async def test_csrf_and_host_boundaries(setup):
    ...
    assert (
        await c.get("/health", headers={"Host": "attacker.example"})
    ).status_code == 400
...
async def test_strict_arguments_and_tool_grants(setup):
    ...
    bad = await rpc(
        ...
        params={
            "name": "ha-api-status",
            "arguments": {"url": "http://metadata.invalid"},
        },
    )
    ...
    names = [
        t["name"]
        for t in (await rpc(c, a["token"], "tools/list", {})).json()["result"]["tools"]
    ]
...
async def test_independent_revocation_and_restart(setup):
    ...
    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=restarted), base_url="http://localhost:4444"
    ) as after:
...
async def test_user_cannot_revoke_or_resolve_anothers(setup):
    ...
    request = (
        await rpc(
            ...
            a["token"],
            params={
                "name": "home_agent_request_connection",
                "arguments": {"service": "homeassistant"},
            },
        )
    ).json()
    link = json.loads(request["result"]["content"][0]["text"])["url"]
    rid = link.split("=")[-1]
    c.cookies.set("jwt_token", session("bob"))
    assert (
        await c.delete("/api/agents/" + a["id"], headers=await headers(c))
    ).status_code == 404
    assert (
        await c.post("/api/requests/" + rid + "/resolve", headers=await headers(c))
    ).status_code == 404
    dashboard = (await c.get("/api/bootstrap")).json()
    assert (
        not dashboard["agents"]
        and not dashboard["requests"]
        and not dashboard["events"]
    )
...
async def test_request_requires_fresh_check_and_never_grants(setup):
    ...
    assert (await rpc(c, a["token"], params={"name": REPOS, "arguments": {}})).json()[
        "error"
    ]["code"] == -32601
...
async def test_mcp_protocol_and_expiry(setup):
    ...
    a = await enroll(c)
    initialize = (
        await rpc(c, a["token"], "initialize", {"protocolVersion": "2025-06-18"})
    ).json()
    assert initialize["result"]["protocolVersion"] == "2025-06-18"
    assert (
        await c.get("/mcp", headers={"Authorization": "Bearer " + a["token"]})
    ).status_code == 405
    assert (
        await c.post(
            "/mcp",
    ...
    with app.state.store.db() as db:
        db.execute("UPDATE agents SET expires=? WHERE id=?", (time.time() - 1, a["id"]))
    assert (await rpc(c, a["token"])).status_code == 401
...
async def test_revocation_survives_broker_failure(setup):
    ...
    def failure(self, request):
        if request.method == "DELETE" and request.url.path.startswith("/tokens/"):
            return httpx.Response(503)
...
async def test_unverified_connections_and_extra_fields_cannot_enroll(setup):
    ...
    assert (
        await c.post(
            "/api/agents",
            headers=h,
            json={"name": "bad", "services": ["gitea"], "days": 1},
        )
    ).status_code == 409
    assert (
        await c.post(
    ...
    assert (
        await c.post(
            "/api/agents",
            headers=h,
            json={"name": "bad", "services": ["admin"], "days": 1},
        )
    ).status_code == 422
