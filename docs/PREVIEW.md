# Running the first application preview

This is a single-operator preview, not a general multi-tenant release. The web app and bounded MCP endpoint are implemented. ContextForge remains the private identity and credential broker. A reviewed, pre-provisioned broker workspace is required; arbitrary service onboarding is not implemented yet.
...
- An explicitly shared Home Assistant availability check. No device control.
...
2. The current adapter expects a private virtual server named `Home Agent Tools Evaluation`, a gateway named `gitea-personal-evaluation`, and tool names `ha-api-status`, `gitea-personal-evaluation-get-me`, and `gitea-personal-evaluation-list-my-repos`. Associate only these three tools with the server. These names are a preview implementation contract, not generic service discovery.
...
7. Sign in through Pocket ID. Verify each connection. Confirm the Gitea username is the intended upstream account. Enroll each agent separately from **Agents**, select its services and expiry, and copy its bearer token directly into the client's protected MCP settings.
...
The endpoint is `<your-origin>/mcp`, using Streamable HTTP with `Authorization: Bearer <enrollment token>`. Tokens are shown once, expire within 1–30 days, and must not be pasted into chat or Git. The app only stores their SHA-256 fingerprints; ContextForge owns the actual credentials and token records.
...
The Python MCP SDK 2.2.0 is the automated protocol client used for this preview. Separate HTTP probes test independent grants. These are **not** evidence of two different commercial runtimes working. Hosted OAuth-only clients, automatic runtime installation, and cross-runtime skill distribution remain unimplemented/unverified.
...
- Home Assistant's underlying shared token can have broader access than the fixed read tool. Use a dedicated account/token before expanding beyond the operator preview. Audit attribution in this app does not make that credential personal upstream.
...
- Refresh behavior is delegated to the pinned broker. Forced expiry/refresh failure and upstream revocation are separate release gates, not guaranteed by one successful connection check.
- There are no arbitrary outbound URL tools, write tools, dynamic integration installs, Docker socket mounts, role-management UI, or public registration in this preview.
- Deployments use a single app process and SQLite. Rate limiting, durable revocation reconciliation, full multi-user authorization, dependency security review, and broader client compatibility are release work.
